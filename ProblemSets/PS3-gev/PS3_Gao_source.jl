...
# γ is a direct measure of the impact of wages on the attractiveness of a job choice. A negative γ indicates that the model finds jobs with higher wages have a lower probability of being chosen.
...
function nested_logit_with_Z(theta, X, Z, y, nesting_structure)
    ...
    for j = 1:J
        if j in nesting_structure[1]
            ...
            num[:, j] =
                lidx[:, j] .* sum(lidx[:, nesting_structure[1]], dims = 2) .^ (lambda[1]-1)
        elseif j in nesting_structure[2]
            # num[:,j] = lidx[:,j] .* (sum of lidx for BC nest)^(lambda[2]-1)
            num[:, j] =
                lidx[:, j] .* sum(lidx[:, nesting_structure[2]], dims = 2) .^ (lambda[2]-1)
        else
            # num[:,j] = lidx[:,j]
            num[:, j] = lidx[:, j]
        end
...
function optimize_nested_logit(X, Z, y, nesting_structure)
    ...
    result = optimize(
        theta -> nested_logit_with_Z(theta, X, Z, y, nesting_structure),
        startvals,
        LBFGS(),
        Optim.Options(g_tol = 1e-5, iterations = 100_000, show_trace = true),
    )
